...
# in the channel. This test only uses sensible heat flux (shf) and shortwave radiation
# (swr) The result is a 2 deg C rise in temperature in the
...
def check_output(idx, test):
    ...
    # confirm that the energy added to the stream results in a 2 deg C rise in temp
    # temperature gradient
    tgrad = tatm - strm_temp
    shf_ener_per_sqm = c_d * rhoa * Cpa * wspd * tgrad
    swr_ener_per_sqm = solr * (1 - shd) * (1 - swrefl)
    ener_transfer = (shf_ener_per_sqm + swr_ener_per_sqm) * (delr * calc_strm_wid)
    ...
    assert np.isclose(df2.loc[0, "RCH1_OUTFTEMP"], strm_temp + temp_rise, atol=2e-6), (
        msg1
    )
