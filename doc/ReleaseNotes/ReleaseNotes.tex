...
6.7.0 & February 3, 2026 & \url{https://doi.org/10.5066/P1IJAXDZ} \\
\hline
\label{tab:releases}
\end{tabular*}
\end{center}
...
\item
The GWT-GWT Exchange requires that the GWF Models corresponding to each GWT model are run concurrently in the same simulation.  There is not an option to read GWF-GWF flows from a previous simulation.  Likewise, the GWE-GWE Exchange also requires that both of the corresponding GWF Models are run concurrently in the same simulation.
...
\item
The Time-Variable Hydraulic Conductivity (TVK) Package is incompatible with the Hydraulic Flow Barrier (HFB) Package, meaning that wherever the TVK Package is used to change hydraulic properties of cells, the conductance between the cell node and the adjacent hydraulic flow barrier will not be recalculated.
...
\item
The PRT Model reports balanced particle mass budgets to the listing file and the binary cell budget file, with budget entries for release, storage, and termination. However, the termination term is aggregated and does not distinguish particle outflows to different types of boundary conditions.
